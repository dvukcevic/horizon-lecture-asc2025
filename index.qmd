...
Optional stopping is safe when your test statistic\
is a **supermartingale under the null**.

## Ville's inequality

Let $S_0, S_1, S_2, \dots$ be a **test supermartingale** (TSM): has
non-negative terms ($S_i \geqslant 0$) and starting value $S_0 = 1$.
...
> The probability that your wealth **ever** exceeds $1 / \alpha$ is at most
> $\alpha$.
...
SPRT: likelihood ratio is a martingale under $H_0$
...
Type 2 error = Waste of resources (and no change of outcome)
...
Unions: $$H_0 = H_0^1 \cup H_0^2 \cup \dots \cup H_0^m$$
$$\rightarrow \text{Need to reject every } H_0^j$$

. . .

Intersections: $$H_0 = H_0^1 \cap H_0^2 \cap \dots \cap H_0^m$$
$$\rightarrow \text{Need to reject at least one } H_0^j$$
...
-   Analysis plans can evolve **flexibly** (infrastructure, budget, etc.)
...
Anoter example: hole-punch filling
